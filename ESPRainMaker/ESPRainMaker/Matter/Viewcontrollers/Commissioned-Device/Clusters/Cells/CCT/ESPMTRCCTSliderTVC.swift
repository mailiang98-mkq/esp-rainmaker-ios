...
@available(iOS 16.4, *)
class ESPMTRCCTSliderTVC: UITableViewCell {
    static let reuseIdentifier: String = "ESPMTRCCTSliderTVC"
    // IB outlets
...
extension ESPMTRCCTSliderTVC: StepSliderProtocol {
    ...
    func setupInitialCCTUI() {
        DispatchQueue.main.async {
            ...
            self.minLabel.text = "\(self.minLevel)"
            self.maxLabel.text = "\(self.maxLevel)"
            ...
            guard let node = self.node, let id = self.deviceId, let levelValue = node.getMatterCCTValue(deviceId: id) else {
                self.setCCTSliderValue(finalValue: 2500.0)
                return
            }
            self.setCCTSliderValue(finalValue: Float(levelValue))
        }
    }
                ...
                self.getColorCluster() { cluster in
                    if let cluster = cluster {
                        cluster.readAttributeColorTemperatureMireds { val, _ in
                            if let val = val {
                                DispatchQueue.main.async {
                                    let cct = Int(1000000/val.intValue)
